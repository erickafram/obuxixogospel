...
// YouTube - Geração de matérias via transcrição de vídeos
const YouTubeTranscriptService = require('./services/YouTubeTranscriptService');

// Página principal do módulo YouTube
app.get('/dashboard/ia/youtube', isAuthenticated, async (req, res) => {
  try {
    const categorias = await Category.findAll({ order: [['nome', 'ASC']] });
    res.render('dashboard/ia/youtube', { 
      user: req.session.user,
      categorias 
    });
  } catch (error) {
    console.error('Erro ao carregar página YouTube:', error);
    res.status(500).send('Erro ao carregar página');
  }
});

// API: Obter transcrição do vídeo
app.post('/api/youtube/transcript', isAuthenticated, async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      transcript: {
        ...
        metadata: transcript.metadata // Incluir metadados do vídeo
      }
    });
  } catch (error) {
...
// API: Analisar tópicos da transcrição
app.post('/api/youtube/analyze-topics', isAuthenticated, async (req, res) => {
  try {
    const { transcript, maxTopics, metadata } = req.body;
    
    if (!transcript) {
      return res.status(400).json({ success: false, message: 'Transcrição é obrigatória' });
    }
    
    console.log('🔍 Analisando tópicos da transcrição...');
    if (metadata) {
      console.log(`📋 Metadados do vídeo: ${metadata.title}`);
    }
    
    // Passar metadados para a análise de tópicos
    const topics = await YouTubeTranscriptService.analyzeTopics(transcript, metadata || {});
    
    // Limitar ao número máximo de tópicos
    const limitedTopics = topics.slice(0, maxTopics || 3);
    
    res.json({ success: true, topics: limitedTopics });
  } catch (error) {
...
// API: Gerar matérias a partir dos tópicos
app.post('/api/youtube/generate-articles', isAuthenticated, async (req, res) => {
  try {
    const { topics, categoria } = req.body;
    
    if (!topics || topics.length === 0) {
      return res.status(400).json({ success: false, message: 'Tópicos são obrigatórios' });
    }
    
    console.log(`📝 Gerando ${topics.length} matéria(s)...`);
    
    const articles = [];
    for (let i = 0; i < topics.length; i++) {
      const topic = topics[i];
      console.log(`📄 Gerando matéria ${i + 1}/${topics.length}: ${topic.title}`);
      
      try {
        const article = await YouTubeTranscriptService.generateArticle(topic, categoria);
        articles.push({
          ...article,
...
// API: Salvar matérias como rascunho ou publicado
app.post('/api/youtube/save-articles', isAuthenticated, async (req, res) => {
  try {
    const { articles, categoria, videoId, publicado } = req.body;
    const slugify = require('slugify');
    
    if (!articles || articles.length === 0) {
      return res.status(400).json({ success: false, message: 'Matérias são obrigatórias' });
    }
    
    const statusTexto = publicado ? 'publicada(s)' : 'como rascunho';
    console.log(`💾 Salvando ${articles.length} matéria(s) ${statusTexto}...`);
    ...
    for (const article of articles) {
      ...
      try {
        ...
        // Usar imagem das sugestões ou imagem padrão do YouTube
        let imagemUrl = '/images/placeholder-youtube.jpg';
        
        // Verificar se há imagens sugeridas na matéria gerada
        if (article.imagensSugeridas && article.imagensSugeridas.length > 0) {
          imagemUrl = article.imagensSugeridas[0].url || article.imagensSugeridas[0];
        } else if (videoId) {
          // Usar thumbnail do YouTube como fallback
          imagemUrl = `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`;
        }
        
        // Truncar campos SEO para respeitar limites do banco
        ...
        // Criar artigo (rascunho ou publicado conforme escolha do usuário)
        const newArticle = await Article.create({
          ...
          categoria: categoria || 'noticias',
          urlAmigavel: slug,
          publicado: publicado === true, // Usar o valor escolhido pelo usuário (padrão: rascunho)
          destaque: false,
          autor: req.session.user?.nome || 'Redação Obuxixo Gospel',
          dataPublicacao: new Date(),
          metaTitulo: metaTitulo,
          metaDescricao: metaDescricao
        });
        
        console.log(`✅ Matéria salva: ${article.titulo} (ID: ${newArticle.id})`);
        
        savedArticles.push({
          id: newArticle.id,
          titulo: newArticle.titulo,
          descricao: newArticle.descricao,
          categoria: newArticle.categoria,
          urlAmigavel: newArticle.urlAmigavel,
          topicTitle: article.topicTitle,
          topicSummary: article.topicSummary
        });
      } catch (saveError) {
        // Log detalhado do erro
        console.error(`❌ Erro ao salvar matéria "${article.titulo}":`, saveError.message);
        if (saveError.errors) {
          saveError.errors.forEach(err => {
            console.error(`   - Campo: ${err.path}, Tipo: ${err.type}, Mensagem: ${err.message}`);
          });
        }
        savedArticles.push({
          error: true,
    ...
    const successCount = savedArticles.filter(a => !a.error).length;
    console.log(`✅ ${successCount} matéria(s) salva(s) com sucesso`);
    
    res.json({ success: true, savedArticles });
  } catch (error) {
